 ...
 */
public final class ScanOCR extends AppCompatActivity implements SurfaceHolder.Callback,
    ...
    private static final String TAG = ScanOCR.class.getSimpleName();
    /**
                    ...
                    if (items.size() != 0) {
                        ...
                        buttonGoToDetails.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                Intent intent = new Intent(ScanOCR.this, MedicineDetails.class);
                        ...
                        Toast.makeText(ScanOCR.this, "Don't found", Toast.LENGTH_SHORT).show();
                    }
    ...
    private void retrievePreferences() {
        ...
        setSourceLanguage(prefs.getString(SettingsActivity.KEY_SOURCE_LANGUAGE_PREFERENCE, ScanOCR.DEFAULT_SOURCE_LANGUAGE_CODE));
        isTranslationActive = prefs.getBoolean(SettingsActivity.KEY_TOGGLE_TRANSLATION, false);

        // Retrieve from preferences, and set in this Activity, the capture mode preference
        isContinuousModeActive = prefs.getBoolean(SettingsActivity.KEY_CONTINUOUS_PREVIEW, ScanOCR.DEFAULT_TOGGLE_CONTINUOUS);
    ...
    private void setDefaultPreferences() {
        ...
        // Continuous preview
        prefs.edit().putBoolean(SettingsActivity.KEY_CONTINUOUS_PREVIEW, ScanOCR.DEFAULT_TOGGLE_CONTINUOUS).apply();

        // Recognition language
        prefs.edit().putString(SettingsActivity.KEY_SOURCE_LANGUAGE_PREFERENCE, ScanOCR.DEFAULT_SOURCE_LANGUAGE_CODE).apply();

        // OCR Engine
        prefs.edit().putString(SettingsActivity.KEY_OCR_ENGINE_MODE, ScanOCR.DEFAULT_OCR_ENGINE_MODE).apply();

        // Autofocus
        prefs.edit().putBoolean(SettingsActivity.KEY_AUTO_FOCUS, ScanOCR.DEFAULT_TOGGLE_AUTO_FOCUS).apply();

        // Disable problematic focus modes
        prefs.edit().putBoolean(SettingsActivity.KEY_DISABLE_CONTINUOUS_FOCUS, ScanOCR.DEFAULT_DISABLE_CONTINUOUS_FOCUS).apply();

        // Beep
        prefs.edit().putBoolean(SettingsActivity.KEY_PLAY_BEEP, ScanOCR.DEFAULT_TOGGLE_BEEP).apply();

        // Character blacklist
        prefs.edit().putString(SettingsActivity.KEY_CHARACTER_BLACKLIST,
                OcrCharacterHelper.getDefaultBlacklist(ScanOCR.DEFAULT_SOURCE_LANGUAGE_CODE)).apply();

        // Character whitelist
        prefs.edit().putString(SettingsActivity.KEY_CHARACTER_WHITELIST,
                OcrCharacterHelper.getDefaultWhitelist(ScanOCR.DEFAULT_SOURCE_LANGUAGE_CODE)).apply();

        // Page segmentation mode
        prefs.edit().putString(SettingsActivity.KEY_PAGE_SEGMENTATION_MODE, ScanOCR.DEFAULT_PAGE_SEGMENTATION_MODE).apply();

        // Reversed camera image
        prefs.edit().putBoolean(SettingsActivity.KEY_REVERSE_IMAGE, ScanOCR.DEFAULT_TOGGLE_REVERSED_IMAGE).apply();

        // Light
        prefs.edit().putBoolean(SettingsActivity.KEY_TOGGLE_LIGHT, ScanOCR.DEFAULT_TOGGLE_LIGHT).apply();
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_flash:
                ...
                mFlash = !mFlash;
                getCameraManager().openFlash(mFlash);
