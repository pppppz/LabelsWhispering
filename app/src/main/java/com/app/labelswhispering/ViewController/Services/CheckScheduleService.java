package com.app.labelswhispering.viewcontroller.services;
...
public class CheckScheduleService extends IntentService {
            ...
            for (i = 0; i < getNumberOfRow; i++) {
                ...
                if (schedule.isAlert()) {
                    if (EatTimeCondition == 1 && schedule.isBeforeMeal() && schedule.isMorning()) {
                        makeAlertScreen(i, schedule.getName(), "Take " + schedule.getAmount() + " tablet and before " + EatTimeConditionTEXT);

                    } else if (EatTimeCondition == 2 && schedule.isAfterMeal() && schedule.isMorning()) {
                        makeAlertScreen(i, schedule.getName(), "Take " + schedule.getAmount() + " tablet and after " + EatTimeConditionTEXT);
    ...
    private void makeAlertScreen(int nId, String title, String body) {
        Log.e(TAG, "text send to alarm activity = " + body);
        it.removeExtra("timeInMillis");
        it.removeExtra("partOfDay");
        Intent intentAlarm = new Intent(this, AlarmScreenActivity.class);
